...
function useColor(color) {
	var colors = ["green", "blue", "red", "yellow", "black", "maroon", "magenta", "cyan"];
	if (colors.indexOf(color) >= 0) {
        ...
        confirm("you said : " + color + " : Which is not the name of a color.");
    }
}

function colorGoogle(myColor) {
  document.getElementById("googleSpeechInput").style.webkitTextFillColor = myColor;
  document.getElementById("googleSpeechInput").style.webkitTextStrokeColor = myColor;
}

window.addEventListener('popstate', function(e) {
	var color = window.location.hash.split("=")[1];
	if (!color) {
		color = "white";
	}
	colorGoogle(color);
